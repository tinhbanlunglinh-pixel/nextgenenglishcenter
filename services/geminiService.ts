...
export const generateLessonPlan = async (
  ...
  const promptCore = `NEXTGEN AI - MASTER ENGLISH PEDAGOGY ENGINE.
  ...
  const promptPractice = `NEXTGEN AI - MASTER EXERCISE GENERATOR & PEDAGOGICAL AUDITOR.
  ...
  ===== 🚨 CRITICAL QUALITY PRINCIPLES (NEXTGEN PEDAGOGICAL STANDARD) 🚨 =====
  PRIORITY: ĐỘ CHÍNH XÁC > CHẤT LƯỢNG CÂU HỎI > ĐỘ PHÙ HỢP TRÌNH ĐỘ > TỐC ĐỘ.
  ...
  // Pass through Nextgen Pedagogical Quality Firewall
...
export const analyzeImageAndCreateContent = async (images: string[], mimeType: string, char: CharacterProfile, mode: AppMode, customPrompt?: string, topic?: string, text?: string): Promise<ContentResult> => {
  ...
  const prompt = `NEXTGEN AI - PEDAGOGICALLY RIGOROUS CREATIVE STORYTELLER.
...
export const extractExamFromDocument = async (
  ...
  const promptExam = `NEXTGEN AI - HIGH FIDELITY EXAM PRESERVATION ENGINE.
...
export const regenerateQuestionWithAi = async (params: RegenerateQuestionParams): Promise<any> => {
  ...
  const prompt = `NEXTGEN AI - MASTER ENGLISH QUESTION AUDITOR & REGENERATOR.
...
===== 🚨 STRICT QUALITY PRINCIPLES (NEXTGEN PEDAGOGICAL STANDARD) 🚨 =====
