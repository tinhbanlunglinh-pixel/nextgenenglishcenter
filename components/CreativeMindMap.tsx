...
export const CreativeMindMap: React.FC = () => {
  ...
  const handleSaveImage = async () => {
    ...
    try {
      ...
      link.download = `nextgen-mindmap-${Date.now()}.png`;
      link.href = dataUrl;
      link.click();
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="w-full bg-white min-h-screen animate-fade-in font-sans pb-20">
      <div className="bg-gradient-to-r from-brand-800 to-[#13443e] text-white p-8 rounded-b-[3rem] shadow-xl mb-8 relative overflow-hidden">
         ...
         <div className="max-w-6xl mx-auto flex flex-col md:flex-row justify-between items-center gap-6">
            <div className="text-center md:text-left">
               <h2 className="text-4xl font-black uppercase tracking-tight mb-2">NEXTGEN ENGLISH</h2>
               <p className="text-brand-100 font-bold opacity-90">Learn English, Lead the way — Nextgen English</p>
            ...
            <div className="flex flex-col items-center md:items-end gap-2 bg-white/10 p-4 rounded-2xl backdrop-blur-sm border border-white/10">
               <span className="text-sm font-semibold">📞 Hotline: 0986 197 229 / 0334 141 989</span>
               <a href="https://www.facebook.com/people/Trung-T%C3%A2m-Ngoa%CC%A3i-Ng%C6%B0%CC%83-Nextgen-U%C3%B4ng-Bi%CC%81/61575042515566/" target="_blank" rel="noopener noreferrer" className="text-sm underline hover:text-brand-200">Fanpage Facebook Nextgen</a>
            </div>
         </div>
      </div>
          ...
          {data && (
            <div className="animate-fade-in flex flex-col items-center">
               <div ref={canvasRef} className="bg-white p-10 rounded-[3rem] shadow-2xl border-[15px] border-slate-50 relative overflow-hidden w-full max-w-[1000px] aspect-[4/3] flex flex-col items-center justify-center">
                  <div className="absolute top-10 right-10 text-brand-600 font-black text-sm uppercase opacity-30">NEXTGEN ENGLISH</div>
