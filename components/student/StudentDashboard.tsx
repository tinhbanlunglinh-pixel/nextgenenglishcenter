...
export const StudentDashboard: React.FC = () => {
  ...
  const groupedAssignments = React.useMemo(() => {
    const filtered = filteredAssignments.filter(assign => {
      const submission = studentName ? getStudentSubmission(assign.id, studentName, currentStudentObj?.id, selectedClassName) : undefined;
  ...
  }, [filteredAssignments, studentName, currentStudentObj, selectedClassName, filterStatus, submissionVersion]);

  const completedCount = React.useMemo(() => {
    return filteredAssignments.filter(a => studentName && getStudentSubmission(a.id, studentName, currentStudentObj?.id, selectedClassName)).length;
  }, [filteredAssignments, studentName, currentStudentObj, submissionVersion, selectedClassName]);
  ...
  if (selectedAssignment && studentName) {
    const effectiveClass = (selectedAssignment.targetClassName && selectedAssignment.targetClassName !== 'Tất cả các lớp' && !selectedAssignment.targetClassName.includes(','))
      ? selectedAssignment.targetClassName
        ...
        <div className="relative z-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
          <div>
            ...
            <h1 className="text-2xl sm:text-4xl font-black uppercase tracking-tight font-display">
              CHÀO MỪNG CON ĐẾN VỚI NEXTGEN ENGLISH!
            </h1>
            <p className="text-brand-100 text-sm sm:text-base font-medium mt-1">
              Xem danh sách bài tập được giao theo ngày, hoàn thành bài tập và nhận ngay chứng nhận điểm cao nhé!
            </p>
          </div>
                          ...
                          return (
                            ...
                            >
                              ...
                              <div className="pt-3 border-t border-slate-100 flex items-center justify-between gap-3">
                                ...
                                >
                                  {hasSubmitted ? '🔒 Xem lại bài (Đã nộp)' : '🚀 Làm bài ngay'}
                                </button>
                              </div>
                            </div>
                          );
