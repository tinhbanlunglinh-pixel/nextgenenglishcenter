...
export const StudentExamView: React.FC<StudentExamViewProps> = ({
  ...
  const existingSubmission = useMemo(() => {
    return getStudentSubmission(assignment.id, studentName, studentId, studentClass);
  }, [assignment.id, studentName, studentId, studentClass]);
  ...
  if (!exam) {
    return (
      <div className="p-8 text-center bg-white rounded-3xl shadow-xl border border-rose-200 space-y-4">
        ...
        <p className="text-slate-600 text-sm">Vui lòng liên hệ với Trung tâm Ngoại Ngữ Nextgen để được hỗ trợ nhé!</p>
        <button
          onClick={onBack}
  ...
  const getEvaluation = (scoreOn10: number) => {
    if (scoreOn10 >= 9) return { text: "XUẤT SẮC", emoji: "🏆", level: "EXCELLENT", praise: "Con làm bài thi tuyệt đỉnh, xứng đáng là ngôi sao sáng của Nextgen English!" };
  ...
  const handleSubmitExam = async () => {
    ...
    if (isSubmitted || existingSubmission) {
      alert('Con đã hoàn thành bài thi này rồi! Link cô giao mỗi học sinh chỉ được làm bài 1 lần duy nhất.');
      return;
    }
    ...
    try {
      ...
      const scoreOn10 = calculated.scoreOn10;
      const finalScore = exam.targetScale === 'original' ? calculated.totalPointsEarned : scoreOn10;
      const evalData = getEvaluation(scoreOn10);
      ...
      const submission: Submission = {
        ...
        score: scoreOn10, // Normalized 10 scale for unified dashboards
        ...
        evaluation: evalData,
        assignmentType: 'exam',
        examScore: calculated.totalPointsEarned,
        examMaxScore: calculated.maxPoints,
        scaledScore10: scoreOn10,
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs sm:text-sm text-amber-800 mt-0.5">
                Link cô giao mỗi học sinh chỉ được làm bài <strong>1 lần duy nhất</strong>. Con đã nộp bài thành công với kết quả: <strong className="text-emerald-700">{existingSubmission?.score ?? gradeResult?.scoreOn10}/10 điểm</strong>. Con có thể xem lại lời giải chi tiết và đáp án bên dưới nhé!
              </p>
            </div>
          </div>
          ...
          <div className="max-w-md mx-auto space-y-3">
            ...
            >
              {isSubmitting ? (
                ...
                <>
                  <span>🚀</span> NỘP BÀI KIỂM TRA CHO NEXTGEN
                </>
              )}
            </button>
          </div>
