...
export const SettingsModal: React.FC<SettingsModalProps> = ({ isOpen, onClose, onSaved }) => {
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setTeacherDisplayName(creds.displayName || 'Giáo viên Nextgen (Nextgen English)');
  ...
  const handleSave = () => {
    ...
    if (activeTab === 'account') {
      ...
      const res = saveTeacherCredentials({
        username: u,
        password: p,
        displayName: teacherDisplayName.trim() || 'Giáo viên Nextgen (Nextgen English)'
      });
          ...
          <div className="overflow-y-auto space-y-4 flex-1 pr-1">
            ...
            <div>
              ...
              <input
                ...
                placeholder="Ví dụ: Nextgen hoặc nextgen"
            ...
            <div>
              ...
              <input
                ...
                placeholder="Ví dụ: Giáo viên Nextgen (Nextgen English)"
                className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl font-bold text-slate-800 text-sm focus:outline-none focus:border-indigo-500"
              />
            </div>
            ...
            <div className="p-3 bg-amber-50 border border-amber-200 rounded-xl text-xs text-amber-800 flex items-start gap-2">
              ...
              <div>
                <span className="font-black">Bảo vệ chống quên mật khẩu:</span> Nếu cô lỡ quên mật khẩu mới đã đổi, cô vẫn luôn có thể đăng nhập bằng tài khoản khôi phục mặc định (<code className="font-mono bg-white px-1 py-0.5 rounded border border-amber-300 font-bold">Nextgen</code> / <code className="font-mono bg-white px-1 py-0.5 rounded border border-amber-300 font-bold">88889999</code>).
              </div>
            </div>
          </div>
        )}
