...
export const StudentManagement: React.FC = () => {
  ...
  // 4b. Xóa toàn bộ học sinh ở tất cả các lớp (để nạp danh sách Nextgen)
  const handleClearAllStudents = async () => {
    const confirmed = confirm(
      '⚠️ XÁC NHẬN XÓA TOÀN BỘ HỌC SINH:\n\n' +
      'Thao tác này sẽ xóa sạch danh sách học sinh của TẤT CẢ các lớp trên hệ thống để trung tâm Nextgen tải lên danh sách học sinh mới.\n\n' +
      '(Lưu ý: Các lớp học vẫn được giữ nguyên, chỉ có danh sách học sinh được dọn sạch).\n\n' +
      'Thầy/cô có chắc chắn muốn thực hiện?'
    );
    if (!confirmed) return;
    ...
    setIsResetting(false);
    refresh();
    showToast('✓ Đã xóa sạch toàn bộ học sinh. Sẵn sàng tải lên danh sách mới của Nextgen!');
  };

  // 4c. Xóa học sinh trong lớp đang chọn
  const handleClearClassStudents = async () => {
    if (!currentClass) return;
    const confirmed = confirm(
      `Cô/thầy có chắc chắn muốn xóa toàn bộ học sinh trong lớp "${currentClass.name}" không?\n\n` +
      `Thao tác này sẽ dọn sạch danh sách lớp để tải lên danh sách học sinh mới từ Excel.`
    );
    if (!confirmed) return;

    await clearStudentsByClass(currentClass.id);
    refresh();
    showToast(`✓ Đã xóa sạch học sinh trong lớp "${currentClass.name}".`);
  };
  ...
  const handleCopyNotDoneZaloList = () => {
    ...
    text += `\nKính nhờ quý phụ huynh nhắc nhở các con tranh thủ vào làm bài và nộp bài để giáo viên Nextgen chấm điểm nhé! Trung tâm cảm ơn phụ huynh ạ! ❤️`;
  ...
  const handleCopyIndividualReminder = (student: Student) => {
    ...
    const title = activeAssignment.title || activeAssignment.topic;
    let text = `Dạ Trung tâm Ngoại Ngữ Nextgen xin gửi lời chào đến phụ huynh em ${student.name}${student.englishName ? ` (${student.englishName})` : ''} ạ!\n`;
    text += `Hiện tại con chưa hoàn thành bài tập "${title}". Nhờ phụ huynh nhắc con mở app làm bài và nộp bài sớm giúp trung tâm nhé! Nextgen xin cảm ơn phụ huynh nhiều ạ! ❤️`;
    navigator.clipboard.writeText(text);
    alert(`Đã copy tin nhắn nhắc nhở cho phụ huynh em ${student.name}!\nThầy/cô có thể dán vào tin nhắn Zalo gửi ngay cho phụ huynh.`);
  };
      ...
      <div className="bg-white rounded-3xl p-6 shadow-xl border border-brand-100 flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <button
            ...
            title="Xóa sạch toàn bộ học sinh của tất cả các lớp để trung tâm Nextgen tải lên danh sách học sinh mới"
          >
            <span>🗑️</span>
            <span>Xóa Toàn Bộ Học Sinh</span>
          </button>
            ...
            <div className="py-12 text-center text-slate-400 space-y-3">
              ...
              <p className="text-sm text-slate-500 max-w-lg mx-auto">
                {homeworkStatusFilter !== 'ALL' ? (
                  ...
                  'Danh sách học sinh đã được dọn sạch. Thầy/cô Trung tâm Nextgen hãy bấm "Nhập từ Excel", "Dán Hàng Loạt" hoặc "Thêm 1 Học Sinh" để nạp danh sách học sinh của mình vào lớp nhé!'
                )}
              </p>
              {homeworkStatusFilter === 'ALL' && (
                <div className="flex justify-center items-center gap-3 pt-2">
                  ...
                  <button
                    type="button"
                    onClick={() => setShowBatchModal(true)}
                    className="px-4 py-2.5 bg-sky-600 hover:bg-sky-700 text-white rounded-xl font-bold text-xs shadow-md transition-all flex items-center gap-1.5"
                  >
                    <span>📋</span> Dán Hàng Loạt
                  </button>
                  <button
                    type="button"
                    onClick={() => setShowAddStudent(true)}
                    className="px-4 py-2.5 bg-brand-500 hover:bg-brand-600 text-white rounded-xl font-bold text-xs shadow-md transition-all flex items-center gap-1.5"
                  >
                    <span>➕</span> Thêm 1 Học Sinh
                  </button>
                </div>
