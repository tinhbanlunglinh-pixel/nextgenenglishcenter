...
import { getClasses, saveAssignment, getAssignments, deleteAssignment, getTodayString, getSubmissions, updateAssignmentTitle } from '../../services/assignmentService';
  ...
  const handleGenerateAI = async () => {
    ...
    if (plannerMode === 'exam') {
      ...
      try {
        ...
        setAssignmentTitle(prev => prev.trim() ? prev : (examData.title || 'Đề kiểm tra tiếng Anh Nextgen'));
      } catch (err: any) {
        setError(err.message || 'Có lỗi xảy ra khi bóc tách đề thi. Thầy/cô hãy kiểm tra lại file đề thi nhé!');
      } finally {
        setLoading(false);
    ...
    try {
      ...
      setLessonPlan(data);
      setAssignmentTitle(prev => prev.trim() ? prev : (data.topic || topic || activeTopic || 'Bài tập tiếng Anh Nextgen'));
    } catch (err: any) {
          ...
          <div className="bg-gradient-to-br from-brand-50 via-white to-emerald-50 p-6 sm:p-8 rounded-3xl border-4 border-brand-300 shadow-2xl space-y-6">
            ...
            <div>
              ...
              <textarea
                ...
                rows={2}
                placeholder="Lời dặn dò của giáo viên dành cho học sinh..."
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs sm:text-sm text-emerald-100 font-medium mt-0.5">
                    Thầy/cô có thể kiểm tra từng từ vựng, ngữ pháp, bài đọc và làm thử các bài tập trước khi giao.
                  </p>
                </div>
              </div>
            ...
            <div className="bg-gradient-to-br from-brand-50 via-white to-emerald-50 p-6 sm:p-8 rounded-3xl border-4 border-brand-300 shadow-2xl space-y-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                <div className="col-span-full">
                  <label className="block text-xs font-black text-slate-700 uppercase tracking-wider mb-1.5">
                    💬 Lời Nhắn Dặn Của Giáo Viên Cho Học Sinh
                  </label>
                  <input
                    type="text"
                    value={teacherNote}
                  ...
                  <button
                    ...
                    onClick={() => {
                      exportAssignmentToWord({
                        id: `assign_${Date.now()}`,
                        title: assignmentTitle.trim() || 'Bài tập tiếng Anh Nextgen',
