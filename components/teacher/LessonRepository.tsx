...
export const LessonRepository: React.FC<LessonRepositoryProps> = ({
  ...
  const handleCopyAssignmentLink = (assign: Assignment) => {
    try {
      ...
      const message = `📢 THÔNG BÁO BÀI TẬP TIẾNG ANH - NEXTGEN ENGLISH
📖 Bài tập: ${assign.title}
🏫 Dành cho: ${classNameStr}
⏰ Hạn nộp: ${deadlineStr}
${assign.teacherNote ? `💬 Lời dặn của giáo viên: "${assign.teacherNote}"\n` : ''}
🔗 LINK LÀM BÀI TRỰC TUYẾN:
${link}

⚠️ QUY ĐỊNH BÀI LÀM:
- Mỗi học sinh CHỈ ĐƯỢC NỘP BÀI 1 LẦN DUY NHẤT.
- Sau khi bấm nộp bài, hệ thống sẽ tự động khóa và lưu điểm vào sổ theo dõi của Nextgen English.
- Các con hãy ôn kỹ bài và kiểm tra thật cẩn thận trước khi nộp nhé!`;

      navigator.clipboard.writeText(message);
      showNotification(`📋 Đã sao chép link và nội dung giao bài cho ${classNameStr}! Thầy/cô có thể dán (Ctrl+V) vào nhóm Zalo của lớp.`);
    } catch (e) {
        ...
        <div className="relative z-10 flex flex-col lg:flex-row lg:items-center justify-between gap-6">
          <div className="space-y-2">
            ...
            <h1 className="text-2xl sm:text-3xl lg:text-4xl font-black uppercase tracking-tight font-display">
              NGÂN HÀNG BÀI TẬP NEXTGEN ENGLISH
            </h1>
            <p className="text-purple-100 text-sm sm:text-base font-medium max-w-2xl leading-relaxed">
              Toàn bộ bài giảng đã soạn được lưu trữ ngăn nắp theo từng lớp và từng ngày. Thầy/cô có thể tìm kiếm, xem lại giáo án đầy đủ, giao lại hoặc nhanh chóng giao bài cho lớp khác chỉ với 1 click!
            </p>
          </div>
        ...
        <div className="p-4 sm:p-6 overflow-y-auto space-y-6">
          ...
          {assignment.teacherNote && (
            <div className="bg-brand-50 border-l-4 border-brand-500 p-4 rounded-2xl">
              <p className="text-xs font-black uppercase text-brand-800">💬 Lời Dặn Của Giáo Viên:</p>
