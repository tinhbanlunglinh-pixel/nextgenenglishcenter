...
export const DailyResultsAggregator: React.FC = () => {
  ...
  const handleDeleteStudentSubmission = (submission: Submission) => {
    if (confirm(`Cô có chắc chắn muốn xóa bài nộp của học sinh "${submission.studentName}" không?\n\nSau khi xóa, học sinh sẽ có thể mở lại link và làm lại bài từ đầu.`)) {
      deleteSubmission(submission.id);
      setViewDetailSubmission(null);
      setSelectedDetailItem(null);
      refresh();
      alert(`Đã xóa bài nộp và mở quyền làm lại cho học sinh "${submission.studentName}". Con có thể mở link và làm bài lại.`);
    }
  };
  ...
  const getSubCountForDate = (targetDate: string) => {
    ...
    if (targetDate === 'ALL') return list.length;
    return list.filter(s => {
      if (!s || !s.submittedAt) return false;
      const subDate = new Date(s.submittedAt);
      if (isNaN(subDate.getTime())) return false;
      const sY = subDate.getFullYear();
      const sM = String(subDate.getMonth() + 1).padStart(2, '0');
      const sD = String(subDate.getDate()).padStart(2, '0');
      return `${sY}-${sM}-${sD}` === targetDate;
    }).length;
  };

  const allCount = getSubCountForDate('ALL');
  const todayCount = getSubCountForDate(todayStr);
  const yesterdayCount = getSubCountForDate(yesterdayStr);
  ...
  const handleCopyIndividualReminder = (item: StudentResultItem) => {
    const assignObj = assignments.find(a => a.id === selectedAssignmentId);
    const assignTitle = assignObj ? (assignObj.title || assignObj.topic) : 'Bài tập tiếng Anh';
    let text = `Dạ Trung tâm Ngoại Ngữ Nextgen xin gửi lời chào đến phụ huynh em ${item.studentName}${item.englishName ? ` (${item.englishName})` : ''} ạ!\n`;
    text += `Hiện tại con chưa hoàn thành bài tập "${assignTitle}". Nhờ phụ huynh nhắc con mở app làm bài và nộp bài sớm giúp trung tâm nhé! Nextgen xin cảm ơn phụ huynh nhiều ạ! ❤️`;
    navigator.clipboard.writeText(text);
                      ...
                      <td className="py-3.5 px-3 text-right">
                        {s ? (
                          <div className="flex items-center justify-end gap-1.5">
                            ...
                            <button
                              onClick={() => handleDeleteStudentSubmission(s)}
                              className="p-1 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition-all text-xs cursor-pointer"
                              title={`Cho phép em ${item.studentName} làm lại (Xóa bài nộp)`}
                            >
                              🗑️
                            </button>
                          </div>
        ...
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl p-6 max-w-lg w-full shadow-2xl space-y-4 max-h-[90vh] flex flex-col">
            ...
            <div className="flex gap-2 pt-2 border-t border-slate-100">
              <button
                onClick={() => handleDeleteStudentSubmission(viewDetailSubmission)}
                className="flex-1 py-2.5 bg-rose-50 hover:bg-rose-100 text-rose-700 font-bold rounded-xl text-xs sm:text-sm border border-rose-200 transition-colors flex items-center justify-center gap-1.5 cursor-pointer"
                title="Xóa bài nộp này để mở quyền cho học sinh làm lại"
              >
                <span>🗑️</span> Cho phép làm lại (Xóa bài nộp)
              </button>
              <button
