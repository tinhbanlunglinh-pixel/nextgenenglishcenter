    ...
    const fallbackMC: ReadingMCQ[] = [
      {
        ...
        options: [
          `Learning English at Nextgen English center`,
          `Playing computer games after school`,
          `Cooking delicious food at home`,
          `Traveling around the world by plane`
        ],
        correctAnswer: 0,
        explanation: `Đoạn văn chủ yếu nói về việc học tiếng Anh tại trung tâm Nextgen ("We learn English at Nextgen English center every day").`
      },
      {
        id: 'rmc_std_2',
...
export const ensureCompletePracticeContent = (
  ...
  const vocabList: VocabularyItem[] = (core.vocabulary && core.vocabulary.length > 0)
    ...
    : [
        ...
        { word: 'teacher', emoji: '👩‍🏫', ipa: '/ˈtiːtʃə/', meaning: 'giáo viên', example: 'Our teachers at Nextgen are wonderful.', sentenceMeaning: 'các thầy cô tại Nextgen thật tuyệt vời.', type: 'noun' },
  ...
  if (scramble.length < 10) {
    ...
    const standardScramble = [
      ...
      { s: "We learn English at Nextgen.", t: "Chúng tôi học tiếng Anh tại Nextgen." },
  ...
  const passage = rawMega?.trueFalsePassage ||
    ...
    (core.reading?.title ? `${core.reading.title}. We learn English every day at Nextgen English center. English is fun and exciting.` : 'We learn English every day at school.');
